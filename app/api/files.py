...
async def upload_file(
    *,
    ...
    s3_data = await upload_file_to_s3(
        file=file, user_id=current_user.id, review_id=review_id, description=description
    )

    file_attachment = crud.file_attachment.create_from_s3_data(db, s3_data=s3_data)

    return FileUploadResponse(
        file=file_attachment, message="File uploaded successfully"
    )
...
def get_review_files(
    *, db: Session = Depends(get_db), review_id: int, skip: int = 0, limit: int = 20
):
    review = crud.review.get(db, id=review_id)
    if not review:
        raise HTTPException(
...
def get_file(*, db: Session = Depends(get_db), file_id: int):
