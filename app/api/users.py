...
from app.schemas.user import User as UserSchema, UserUpdate, UserResponse, UserAccountManage, PasswordChange, EmailChangeRequest, EmailChangeConfirm
...
async def request_email_change(
    ...
    if not crud.user.authenticate(db, email=current_user.email, password=email_data.password):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def confirm_email_change(
    ...
    verification = crud.user.verify_email_change(
        db=db, 
        user_id=current_user.id, 
        verification_code=confirm_data.verification_code
    )
