...
class CRUDAccountSettings(
    CRUDBase[AccountSettings, AccountSettingsUpdate, AccountSettingsUpdate]
):
    def get_by_user_id(self, db: Session, *, user_id: int) -> Optional[AccountSettings]:
        return (
            db.query(AccountSettings).filter(AccountSettings.user_id == user_id).first()
        )
