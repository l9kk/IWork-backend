...
class CRUDFileAttachment(CRUDBase[FileAttachment, FileAttachmentCreate, FileAttachmentUpdate]):
    def create_from_s3_data(self, db: Session, *, s3_data: Dict[str, Any]) -> FileAttachment:
    ...
    def get_user_files(
        ...
        ).order_by(FileAttachment.created_at.desc()).offset(skip).limit(limit).all()

    def get_review_files(
            self, db: Session, *, review_id: int, skip: int = 0, limit: int = 100
    ) -> list[Type[FileAttachment]]:
        return db.query(FileAttachment).filter(
            FileAttachment.review_id == review_id
        ).order_by(FileAttachment.created_at.desc()).offset(skip).limit(limit).all()
